...
int check(vector<int> &pre, int sum)
{
    int low = 0;
    int high = pre.size() - 1;
    int ans = 0;
...
void solvee()
{
    ...
    {
        currSum += v[i];
        int ind = check(pre, pre[n - 1] - currSum);
        int otherSum = pre[n - 1] - pre[ind];
        // cout << currSum << " " << otherSum << " " << ind << " " << v[ind] << endl;
        if (ind >= i && otherSum == currSum)
        {
            ans = max(ans, (i + 1) + (n - 1) - ind);
        }
    }
    cout << ans << endl;
}
