...
void solvee()
{
    ...
    for (int i = n; i >= 1; i--)
    {
        int framedSegment = i % m;
        framedSegment = mod_mul(framedSegment, fact[n - i + 1], m);
        framedSegment = mod_mul(framedSegment, fact[i - 1], m);
        framedSegment = mod_mul(framedSegment, i, m);
        ans = mod_add(ans, framedSegment, m);
    }
    cout << ans << endl;
}
